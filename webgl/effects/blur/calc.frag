...
void main(void){
  ...
  vec3 color=vec3(
    dot(texture2D(texture,texcoord+v1),vec4(0.96, 0.01, 0.03, 0)),
    dot(texture2D(texture,texcoord+v2),vec4(0.03, 0.96, 0.01, 0)),
    dot(texture2D(texture,texcoord+v3),vec4(0.01, 0.03, 0.96, 0))
  );
